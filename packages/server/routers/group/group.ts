    ...
    info: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            if (member == null) {
                throw new TRPCError({
                    code: "NOT_FOUND",
                    message: "Vous n'êtes pas encore membre de ce groupe",
                });
            }
    ...
    joinByUniqueName: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            if (!group.public)
                throw new TRPCError({
                    code: "BAD_REQUEST",
                    message: "Le groupe n'est pas un groupe public",
                });
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            if (group.owner_id === ctx.session.user.id)
                throw new TRPCError({
                    code: "BAD_REQUEST",
                    message:
                        "Le propriétaire du groupe ne peut pas quitter le groupe, veuillez transférer vos autorisations avant de le quitter",
                });
